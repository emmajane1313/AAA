...
const Metadata: FunctionComponent<MetadataProps> = ({
  ...
  switch (metadata) {
    case "ImageMetadata":
      return (
        <div className="relative w-full h-44 flex flex-row gap-2 items-start justify-start">
          <div className="relative w-full h-full max-h-44 overflow-y-scroll p-1.5 items-start justify-start bg-ligero">
            {(data as ImageMetadata)?.content as string}
          </div>
          <div className="relative w-full h-full flex items-start justify-start">
            <div
              className="relative w-full h-full flex items-start justify-start cursor-pointer"
              onClick={() =>
                setImageView(
                  `${INFURA_GATEWAY}/ipfs/${
    ...
    case "TextOnlyMetadata":
      return (
        <div className="relative w-full h-fit max-h-32 overflow-y-scroll p-1.5 items-start justify-start bg-ligero">
